...
not_pax_fault:
	...
	if (address + 65536 + 32 * sizeof(unsigned long) < regs->sp) {
		if (error_code & PF_USER)
			goto bad_area;
		printk(KERN_ERR "PAX: abnormal kernel stack expansion: %s(%d) %lx/%lx %lx %lx-%lx\n", tsk->comm, task_pid_nr(tsk), regs->sp, task_pt_regs(tsk)->sp, address, vma->vm_start, vma->vm_end);
		dump_stack();
	}
